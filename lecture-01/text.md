...
Вам это не понадобится — принцип отказа от добавления функциональности, в которой нет непосредственной надобности.

Это важно на этапе планирования и проектирования. Если какая-то предположительная фича понадобится только через шесть месяцев, то не стоит ее реализоывать сейчас. И если на ее реализацию надо два месяца, то не надо приступать к ней в ближайшие четыре месяца.

Делать фичу стоит, когда уже есть хорошо сформулированные требования, потому что переделывать потом будет дороже, да и на поддержание ненужной фичи тоже уходят ресурсы. Лучше потратить время на что-нибудь другое полезное.

Но тут есть один тонкий момент: принцип YAGNI не стоит применять к усилиям по упрощению кода для более легкой модификации.
...
* открыты для расширения: поведение может быть расширено путём создания новых типов сущностей.
...
Интерфейс класса должен быть зафиксирован, можно добавлять что-то новое, но нельзя ломать обратную совместимость.

- ## LSP (Liskov Substitution Principle)

Функции, которые используют базовый тип, должны иметь возможность использовать подтипы базового типа, не зная об этом.

Если Circle и Rectangle являются подтипами Shape, то при подстановке их в любое место, где используется Shape, код должен оставаться рабочим и корретным.
...
про преимущества декларативной логики перед императивной
...
const anchorLinks = [
  {
    text: 'Программы',
    path: '#programs',
    isPresent: isNotEmptyArray(this.props.programs),
  },
  {
    text: 'Проекты',
    path: '#projects',
    isPresent: isNotEmptyArray(this.props.prprojects),
  },
  {
    text: 'Клиенты',
    path: '#clients',
    isPresent: isNotEmptyArray(this.props.clients) || isNotEmptyArray(this.props.extended_clients),
  },
  {
    text: 'Образование',
    path: '#education',
    isPresent: isNotEmptyArray(this.props.education),
  },
  {
    text: 'Видео',
    path: '#video',
    isPresent: isNotEmptyArray(this.props.videos),
  },
];
```
...
```js
document.getElementById('iPAN-Mask').addEventListener('keyup', function(e) {
  const ignoreKeyCodes = [8, 37, 39]; // backspace, left arrow, right arrow, delete
